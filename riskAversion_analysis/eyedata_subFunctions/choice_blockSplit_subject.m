...
set(gcf, 'position', [6.8792 5.2070 16.6793 9.4827]);
h1 = subplot(1, 2, 1);
h2 = subplot(1, 2, 2);
    ...
    for isubject = 1: length(subs)
        ...
        for itype = 1:2
            ...
            for icnd = 1:2
                ...
                medPupil{it, isubject}(itype, icnd)     = median(data2run.choice_prtileResp_pupil);

                splitIdx{it}{isubject, itype}{icnd, 1}  = data2run.choice_prtileResp_pupil < medPupil{it, isubject}(itype, icnd);
                splitIdx{it}{isubject, itype}{icnd, 2}  = data2run.choice_prtileResp_pupil > medPupil{it, isubject}(itype, icnd);

                tmpData                             = [];
                tmpBin                              = [];
                tmpt                                = [];
                tmpData                             = data2run(:, [1:9, 19]);
...
riskPref_split = fitglme(binned_all, 'propRisky ~ cndIdx*distIdx*pupilBin*blockSplit + (1|subIdx) + (1|subIdx:cndIdx:distIdx)')
riskBias_split = fitglme(binned_all, 'riskBias ~ cndIdx*distIdx*pupilBin*blockSplit + (1|subIdx) + (1|subIdx:cndIdx:distIdx)')
an_1 = anova(riskPref_split);
an_2 = anova(riskBias_split);
C = dataset2cell(an_1);
anovaWriteTable(C, 'riskPref_blockSplit');
C1 = dataset2cell(an_2);
anovaWriteTable(C1, 'riskbias_blockSplit');
