function [newpupil] = blink_regressout(pupildata, fsample, blinksmp, saccsmp, sampleinfo, plotme, addBackSlowDrift, figFile)
...
if plotme,
    clf;
    if plotme == 2
        figure('Position',[20000 20000 800 600])
    else figure
    end
%     figure
%     if plotme == 2, set(gcf,'position',[10000 10000 800 600]), end  % if figure shouldn't be shown but rather saved to file
    subplot(6,1,1); plot(dat.time,dat.pupil);
    axis tight; box off; ylabel('Interp');
    set(gca, 'xtick', []);
end

% filter the pupil timecourse twice first, get rid of slow drift
[b,a]      = butter(3, freqs(1)*2/fsample, 'high');

if size(sampleinfo,1) == 1  % if only one block
    dat.hpfilt = filtfilt(b,a, dat.pupil); % filter with zero lag
else   % if multiple blocks, filter each time-series separately
    for block = 1:size(sampleinfo,1)
        dat.hpfilt(sampleinfo(block,1):sampleinfo(block,2)) = filtfilt(b,a, dat.pupil(sampleinfo(block,1):sampleinfo(block,2)));
    end
end
...
[b,a] = butter(3, freqs(2)*2/fsample, 'low');
if size(sampleinfo,1) == 1  % if only one block
    dat.bpfilt = filtfilt(b,a, dat.hpfilt);
else   % if multiple blocks, filter each time-series separately
    for block = 1:size(sampleinfo,1)
        dat.bpfilt(sampleinfo(block,1):sampleinfo(block,2)) = filtfilt(b,a, dat.hpfilt(sampleinfo(block,1):sampleinfo(block,2)));
    end
end    

if plotme,
    subplot(6,1,2); plot(dat.time,dat.bpfilt);
...
for r = 1:2, % two regressors
    ...
    n_events(r) = length(thissmp);  % storing number of events
    
    % shift the starting points so the deconvolution catches all samples
...
% ====================================================== %
% STEP 4: FIT IRF GAMMA FUNCS
% ====================================================== %

% double Erlang gamma function from Hoeks and Levelt, Wierda
x = linspace(0, range(imp), numel(deconvolvedPupil(:, 1)))';

% baseline deconvolved IRFs (using first 200ms post-event) before fitting
deconvolvedPupil(:, 1) = deconvolvedPupil(:, 1) - mean(deconvolvedPupil(x>=0 & x<=0.2,1));
deconvolvedPupil(:, 2) = deconvolvedPupil(:, 2) - mean(deconvolvedPupil(x>=0 & x<=0.2,2));
...
if plotme,
    subplot(6,1,4); plot(dat.time,prediction);
...
if plotme == 2, saveas(gcf,figFile,'tiff'); close all, end
