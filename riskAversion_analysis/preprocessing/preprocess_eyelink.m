function [trl] = preprocess_eyelink(raw_path, ptIdx, blockNum)
    ...
    if ~exist(tmpFilename)
        ...
        % identify number of blinks during the sEBR epoch
        if blockNum ~= 4
            sbrStart    = find(data.blinksmp(:, 1) >= tmptrl(121).encodes(1, 2), 1);
            sbr_blinks  = data.blinksmp((sbrStart: end), :);
            sbr_dur     = sbr_blinks(:, 2) - sbr_blinks(:, 1);
    
            tmpSavename = ['SBRblock_' num2str(blockNum) '.mat'];
            save(tmpSavename, 'sbr_blinks');
        end
...
%         dwnTime  = (0:(length(pupil)-1))./500;
        dwnTime = resample(data.times, 50, 1000)';
